...
/* Construye el HTML del PNG de Inicio (Resumen) a partir de los filtros
   ...
   downloadDashboardPNG() para poder reutilizarlo tal cual en la vista
   previa del menú de filtros (openPngExportInicioModal), así la previsualización
   muestra siempre datos reales — nunca un maquetado aparte. */
function buildDashboardPngHTML(){
  const tareas=filteredTareas(),aud=filteredAuditorias();
...
/* ════════════════════════════════════════════════════════════════════
   MENÚ DE FILTROS + VISTA PREVIA — solo el PNG de Inicio (Resumen)
   Tareas / Auditorías / Historial siguen descargando directo con el
   filtro global de arriba, tal como antes: este menú es exclusivo de
   Inicio. Reutiliza los MISMOS campos que lee getFilterState() — los
   valores elegidos aquí se copian de forma temporal a los selects reales
   (#f-razon/#f-centro/#f-tienda/#f-desde/#f-hasta), se genera el PNG con
   datos reales, y al terminar se restauran los filtros originales para
   no alterar lo que el usuario tenía seleccionado en el resto del panel. */
function pngmSnapshotFiltros(){
  return{
    razon:document.getElementById('f-razon').value,
    centro:document.getElementById('f-centro').value,
    tienda:document.getElementById('f-tienda').value,
...
function pngmAplicarFiltros(v){
  document.getElementById('f-razon').value=v.razon;
  document.getElementById('f-centro').value=v.centro;
  document.getElementById('f-tienda').value=v.tienda;
  document.getElementById('f-tipo').value=v.tipo;
  document.getElementById('f-desde').value=v.desde;
  document.getElementById('f-hasta').value=v.hasta;
}
function pngmRefrescarCentroTienda(){
  const razon=document.getElementById('pngm-razon').value;
  const filtA=STORE.tareas.filter(t=>razon==='ALL'||t.razon===razon);
  const filtAud=STORE.auditorias.filter(a=>razon==='ALL'||a.razon===razon);
  const centros=limpiarOpciones(uniq([...filtA.map(t=>t.centro),...filtAud.map(a=>a.centro)]));
  const tiendas=limpiarOpciones(uniq([...filtA.map(t=>t.tienda),...filtAud.map(a=>a.tienda)]));
  const fc=document.getElementById('pngm-centro'),ft=document.getElementById('pngm-tienda');
  const cv=fc.value,tv=ft.value;
  fc.innerHTML='<option value="ALL">Todos</option>'+centros.map(c=>`<option>${c}</option>`).join('');
  ft.innerHTML='<option value="ALL">Todas</option>'+tiendas.map(t=>`<option>${t}</option>`).join('');
  if([...fc.options].some(o=>o.value===cv))fc.value=cv;
...
function pngmValoresModal(){
  return{
    razon:document.getElementById('pngm-razon').value,
    centro:document.getElementById('pngm-centro').value,
    tienda:document.getElementById('pngm-tienda').value,
    tipo:'ALL',
    desde:document.getElementById('pngm-desde').value,
    hasta:document.getElementById('pngm-hasta').value
  };
}
function pngmActualizarPreview(){
  const frame=document.getElementById('pngm-frame');
  if(!frame)return;
  const original=pngmSnapshotFiltros();
  pngmAplicarFiltros(pngmValoresModal());
  const inner=buildDashboardPngHTML();
  pngmAplicarFiltros(original); // restaurar de inmediato: no afectar el resto del panel
  frame.srcdoc=`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${PNG_CSS}</style></head>
    <body><div class="wrap" id="cap">${inner}</div></body></html>`;
  frame.onload=function(){
    try{
      const cap=frame.contentDocument.getElementById('cap');
      const w=cap.scrollWidth,h=cap.scrollHeight;
      const scale=Math.min(1,440/w);
      frame.style.width=w+'px';frame.style.height=h+'px';
      frame.style.transform='scale('+scale+')';
      const wrap=document.getElementById('pngm-preview-wrap');
      if(wrap){wrap.style.width=Math.round(w*scale)+'px';wrap.style.height=Math.min(Math.round(h*scale),460)+'px';}
    }catch(e){/* ignorar: la previsualización es de mejor esfuerzo */}
  };
}
function openPngExportInicioModal(){
  const f=pngmSnapshotFiltros();
  const razonOpts=document.getElementById('f-razon').innerHTML;
  const html=`<div class="png-export-body">
      <div class="png-export-filters">
        <div class="fg"><label>Razón</label><select id="pngm-razon" onchange="pngmRefrescarCentroTienda();pngmActualizarPreview()">${razonOpts}</select></div>
        <div class="fg"><label>Centro</label><select id="pngm-centro" onchange="pngmActualizarPreview()"></select></div>
        <div class="fg"><label>Sucursal / Tienda</label><select id="pngm-tienda" onchange="pngmActualizarPreview()"></select></div>
        <div class="fg"><label>Desde</label><input type="date" id="pngm-desde" onchange="pngmActualizarPreview()"></div>
        <div class="fg"><label>Hasta</label><input type="date" id="pngm-hasta" onchange="pngmActualizarPreview()"></div>
      </div>
      <div class="png-export-preview">
        <div>
          <div id="pngm-preview-wrap" class="png-preview-wrap"><iframe id="pngm-frame" class="png-preview-frame"></iframe></div>
          <div class="png-preview-hint">Vista previa con datos reales. Ajusta los filtros y se actualiza.</div>
        </div>
      </div>
    </div>`;
  openModal('🖼️ Descargar PNG — Resumen',html,[
    {label:'Cancelar',cls:'btn-ghost',fn:closeModal},
    {label:'🖼️ Descargar PNG',cls:'btn-teal',fn:pngmDescargarDesdeModal}
  ]);
  document.getElementById('pngm-desde').value=f.desde;
  document.getElementById('pngm-hasta').value=f.hasta;
  document.getElementById('pngm-razon').value=f.razon;
  pngmRefrescarCentroTienda();
  document.getElementById('pngm-centro').value=f.centro;
  document.getElementById('pngm-tienda').value=f.tienda;
  pngmActualizarPreview();
}
function pngmDescargarDesdeModal(){
  const original=pngmSnapshotFiltros();
  pngmAplicarFiltros(pngmValoresModal());
  downloadDashboardPNG();
  pngmAplicarFiltros(original);
  closeModal();
}
...
function setView(v){
  ...
  /* La barra de filtros globales (Razón/Centro/Tienda/Tipo/Fechas/Mes/Rango)
     ...
     existiendo en el DOM (solo ocultos) porque otras funciones (PNG,
     exportCSV, filteredTareas/filteredAuditorias) siguen leyendo sus
     valores; los menús de filtro de los botones PNG los actualizan de forma
     temporal y los restauran al terminar. Se oculta también el data-strip
     de importación (Cargar Excel/Generar) porque ese flujo alimenta las
     mismas dos colecciones (STORE.tareas/STORE.auditorias) y solo aplica
     a Inicio. */
  var _barraFiltrosVistas=(v==='dash');
...
function filteredAudByView(){
  ...
  /* #aud-f-tipo es un <select> oculto: solo lo mueven los menús de filtro
     de los botones PNG (openAuditoriasPngMenu). Si no existe (por si algún
     día se quita del HTML) se ignora sin romper el filtrado normal. */
  var tipoEl=document.getElementById('aud-f-tipo');
  var tipo=tipoEl?tipoEl.value:'ALL';
  return STORE.auditorias.filter(function(a){
    ...
    if(tipo!=='ALL'&&norm(a.clase||'')!==norm(tipo))return false;
    return true;
  });
}
...
function downloadAuditoriasPNG(){
  ...
  return html2canvas(el,{scale:2,backgroundColor:'#fff'}).then(function(canvas){
...
function historialAuditoriasPorSucursalHTML(filtroExtra){
  ...
  /* filtroExtra viene del menú emergente del botón "PNG Historial por
     sucursal" (openHistorialPngMenu): permite acotar por mes y por tipo/
     clase de auditoría además de la tienda del filtro global de arriba. */
  var fe=filtroExtra||{};
...
/* ════════════════════════════════════════════════════════════════════
   MENÚS DE FILTRO DE LOS BOTONES PNG (Auditorías / Historial / Sucursales)
   Ahora que la barra de filtros global solo vive en Inicio, estos tres
   botones abren su propio menú emergente con Tienda / Mes / Tipo de
   auditoría antes de generar la imagen. Copian los valores elegidos a
   los <select> reales que cada generador ya sabe leer (#aud-f-tienda,
   #aud-f-mes, #aud-f-tipo para Auditorías/Historial; #f-tienda, #f-gran,
   #f-tipo para Sucursales/Pendientes), generan el PNG y al terminar
   restauran los valores originales para no alterar el resto del panel.
════════════════════════════════════════════════════════════════════ */
function pngMenuOptsAuditorias(){
  var tiendas=limpiarOpciones([...new Set(STORE.auditorias.map(function(a){return a.tienda;}).filter(Boolean))]).sort();
  var mesesVistos={};
  STORE.auditorias.forEach(function(a){
    if(!a.mes)return;
    var k=norm(a.mes);
  ...
  var meses=Object.values(mesesVistos).sort(function(a,b){return mesIndexFromNombre(a)-mesIndexFromNombre(b);});
  return{
    tiendaHTML:'<option value="ALL">Todas</option>'+tiendas.map(function(t){return '<option>'+t+'</option>';}).join(''),
    mesHTML:'<option value="ALL">Todos</option>'+meses.map(function(m){return '<option>'+m+'</option>';}).join('')
  };
}
var TIPO_AUDITORIA_HTML='<option value="ALL">Todas</option>'+
  '<option value="AUDITORIAS DE COLABORACION">Auditorías de Colaboración</option>'+
  '<option value="AUDITORIA ORDEN Y LIMPIEZA">Orden y Limpieza</option>'+
  '<option value="AUDITORIA CARTERA">Cartera</option>';
function pngMenuFieldsHTML(tiendaHTML,mesHTML,tipoHTML){
  return '<div class="png-export-body"><div class="png-export-filters" style="display:grid;grid-template-columns:1fr;gap:12px">'+
    '<div class="fg"><label>Tienda / Sucursal</label><select id="pngmenu-tienda">'+tiendaHTML+'</select></div>'+
    '<div class="fg"><label>Mes</label><select id="pngmenu-mes">'+mesHTML+'</select></div>'+
...
function pngMenuGenerarHistorial(){
  ...
  var original=document.getElementById('f-tienda').value;
  document.getElementById('f-tienda').value=tienda;
  var btn=null;
  closeModal();
